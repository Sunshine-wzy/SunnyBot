...
class AntiRecall {
    ...
    private suspend fun doClearJob() {
        PluginMain.logger.info("聊天记录定时清除,running")
        groupMap.forEach{
            ...
            PluginMain.logger.info("群${it.key} 聊天记录缓存数为${list.size}")
        }
        PluginMain.logger.info("聊天记录定时清除,done")
    }
    ...
    suspend fun antiRecallByGroupEvent(event: MessageRecallEvent.GroupRecall) {
        ...
        mutex.withLock {
            ...
            if(!flagFindSuccess)
                event.group.sendMessage("${event.author.nameCardOrNick} 撤回了一条消息")
